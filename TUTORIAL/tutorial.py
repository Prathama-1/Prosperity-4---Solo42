...
def tomatoes_orders(order_depth: OrderDepth, position: int) -> List[Order]:
    """
    Two-step logic every tick:

      STEP 1 — TAKE (only strong signals)
        We only take aggressively if BOTH the best AND second-best level
        are on the same side of mid.
        ► DATA: Level 2 has avg vol = 19.6 (vs level 1 = 7.5).
                If ask2 is ALSO below mid, this is a real selling wave,
                not just a thin quote someone placed to mislead us.
        ► WHY:  Without this filter, we'd chase every tiny wiggle and
                build up a position in the wrong direction on trending days.

      STEP 2 — MAKE (the main edge)
        Post bid = best_bid + 1  (1 tick inside the market)
        Post ask = best_ask - 1  (1 tick inside the market)
        Skew both by -0.2 × position.
        ► WHY:  The spread is 13–14 ticks. We tighten it to 11–12.
                Any participant hitting the market fills US before the bot.
                We earn ~5–6 ticks each side vs true mid.
        ► WHY NO PREDICTION:
                We do NOT try to predict where Tomatoes will go.
                The return autocorrelation is -0.41 (mean-reverting),
                meaning any move tends to reverse. The best play is to
                sit in the middle and collect the spread both ways.
                Inventory skew handles the drift problem automatically.
    """
    orders   : List[Order] = []
    buy_vol  = 0
    sell_vol = 0
    ...
    # Guard: we always have data (confirmed 0 empty rows), but be safe
    if not order_depth.sell_orders or not order_depth.buy_orders:
        return orders

    ask_levels = sorted(order_depth.sell_orders.keys())          # cheap → expensive
    bid_levels = sorted(order_depth.buy_orders.keys(), reverse=True)  # expensive → cheap
    ...
    # ── STEP 1: TAKE — only when level 2 confirms ─────────────────────────
    # ► Example: bid1=4982, ask1=4995, mid=4988.5
    #   ask2=4994 < mid-1 (4987.5)? No → don't take.
    #   ask2=4985 < mid-1 (4987.5)? Yes → real selling pressure → BUY.

    if len(ask_levels) >= 2:
        ask2 = ask_levels[1]
        if ask2 < mid - 1:                          # level 2 also below mid
            can_buy = limit - position - buy_vol
            qty     = min(best_ask_vol, can_buy)
            if qty > 0:
    ...
    if len(bid_levels) >= 2:
        bid2 = bid_levels[1]
        if bid2 > mid + 1:                          # level 2 also above mid
            can_sell = limit + position - sell_vol
            qty      = min(best_bid_vol, can_sell)
            if qty > 0:
                orders.append(Order("TOMATOES", best_bid, -qty))
                sell_vol += qty

    # ── STEP 2: MAKE — 1 tick inside market, skewed by inventory ──────────
    # Skew example: position = +40 → skew = int(0.2 × 40) = 8
    #   → bid = best_bid + 1 - 8 = best_bid - 7  (we quote further from mid)
    #   → ask = best_ask - 1 - 8 = best_ask - 9  (we quote closer to mid → sell sooner)
    skew_ticks = int(round(TOMATOES_SKEW * (position + buy_vol - sell_vol)))

    our_bid = (best_bid + TOMATOES_INSIDE) - skew_ticks
    our_ask = (best_ask - TOMATOES_INSIDE) - skew_ticks

    # Safety: if skew pushes quotes to cross each other, pin them around mid
    if our_bid >= our_ask:
        our_bid = int(mid) - 1
        our_ask = int(mid) + 1
